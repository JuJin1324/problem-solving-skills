...
7. 설계 스킬 완료 후 구현 스킬 착수 전에는 개인 숙고(예: 짧은 산책/메모 정리) 또는 동료 설계 검토 시간을 가진 뒤 `GO`를 결정한다.
...
6. 설계 단계에서 구현 단계로 넘어가는 경우 설계 숙성 게이트 수행 여부를 확인한다.
7. 전환 조건 충족 후에만 다음 단계로 라우팅한다.
...
## 변경 이력
- `v2.5.0` (2026-03-06): 설계 스킬 이후 구현 전 전환 시 개인 숙고/동료 검토 기반 설계 숙성 게이트를 운영 원칙과 체크리스트에 추가함.
