...
- 확정된 결정: 기간 `{시작일}~{종료일}`, US 실행 순서, DoD 최소 기준.
- 확정된 결정: 각 US 완료 목표일(예: `US-{N}.1={YYYY-MM-DD}`)과 의존 관계.
- 바로 실행할 내용: US-{N}.1 착수 태스크, 첫 Checkpoint 시점, 선행 ADR 작성 여부.
...
| 순서 | US | 완료 목표일 | 의존 관계 |
|------|----|----|----|
| 1 | US-{N}.1: {목표} | {YYYY-MM-DD} | 없음 |
| 2 | US-{N}.2: {목표} | {YYYY-MM-DD} | US-{N}.1 완료 후 |
| 3 | US-{N}.3: {목표} | {YYYY-MM-DD} | US-{N}.2 완료 후 |
