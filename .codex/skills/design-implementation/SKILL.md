...
핵심 원칙:
- 바퀴를 새로 만들지 않는다: 기술 스택 결정 전 `기존 제공 기능 -> 검증된 라이브러리/서비스 -> 직접 구현` 순서로 검토한다.
- 구현 전에 설계 증적을 남긴다.
...
- 설계 완료 후 구현(`execute-implementation`) 착수 전 개인 숙고(예: 짧은 산책/메모 정리) 또는 동료 설계 검토 시간을 가진다.
...
### 2단계. 재사용 후보 탐색 (Reuse-first)
- 구현 목적에 맞는 라이브러리/서비스 후보를 검색으로 수집
- `tech-stack.md`를 사용하기로 했다면 해당 문서 내용을 기준으로 검색 키워드를 구성
- 수집 후보 중 재사용 가능한 옵션을 우선 검토하고, 아래 우선순위를 적용한다:
  - 1순위: 현재 프로젝트/선정 스택에서 이미 제공하는 기능
  - 2순위: 공식 생태계(프레임워크/검증된 라이브러리)에서 제공하는 기능
  - 3순위: 직접 구현(예외)
- 직접 구현은 아래 조건을 모두 충족할 때만 허용한다:
  - 재사용 후보가 없거나 핵심 요구사항을 충족하지 못함
  - 라이선스/보안/운영 제약 등 채택 불가 사유가 명시됨
  - `tech-stack.md`의 후보 비교/제외 사유에 증적이 기록됨
...
### 10단계. 설계 숙성 게이트
- `execute-implementation`으로 넘어가기 전에 아래 중 최소 1개를 수행한다:
  - 개인 숙고: 짧은 산책/메모 정리로 설계의 빈틈/과설계 여부 점검
  - 동료 검토: 동료와 경계/트레이드오프/실행 위험을 빠르게 점검
- 숙성 게이트 결과(`GO/HOLD`, 핵심 코멘트 1~3개)를 설계 문서 또는 연계 문서에 기록한다.

## 안티패턴
- 라이브러리/서비스 탐색 없이 직접 구현부터 시작
- 기존 프레임워크가 제공하는 기능을 확인하지 않고 커스텀 유틸/인프라를 먼저 만드는 방식
...
## 다음 단계
- `execute-implementation` 착수 전 설계 숙성 게이트를 먼저 통과한다.
