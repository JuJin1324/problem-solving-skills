...
- 테스트 구현은 `기존 테스트 스택/유틸 -> 검증된 라이브러리 -> 직접 구현` 순서로 선택한다.
...
- 재사용 우선 검토:
  ...
  - 3순위: 직접 구현(예외)
- 직접 구현은 재사용 대안 부재 또는 핵심 요구 미충족이 증적으로 확인될 때만 허용하고 사유를 실행 결과 문서에 기록한다.
...
- 실패 로그 없이 “실패”만 기록
- 결함을 구현/설계로 피드백하지 않음
- 기존 테스트 스택으로 가능한 검증을 커스텀 테스트 유틸/러너로 먼저 대체하는 방식
