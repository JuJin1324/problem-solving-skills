# Kotlin DDD + Clean Architecture 코드 컨벤션 (초안)

## 1) 목적
- 백엔드 Kotlin 구현에서 도메인 규칙을 프레임워크/DB/UI와 분리한다.
- DDD 경계(애그리거트, 리포지토리, 도메인 이벤트)를 코드 규칙으로 강제한다.
- 포맷/정적분석/아키텍처 테스트를 CI에 포함해 일관성을 자동화한다.

## 2) 적용 범위
- 언어: Kotlin 1.9+ (권장), JDK 17+
...
│  │  ├─ out/persistence/         # JPA/R2DBC Adapter
...
- Value Object는 `@JvmInline value class` 또는 불변 `data class`로 구현하고 생성 시 검증한다.
- 도메인 예외는 기술 예외를 숨기고 비즈니스 용어로 명명한다.
- 도메인 계층에 Spring 어노테이션/프레임워크 의존을 두지 않는다.

### Service (UseCase)
- 유스케이스는 오케스트레이션 전담, 핵심 비즈니스 규칙은 Domain으로 위임.
- 입력/출력 모델은 `data class` 또는 `value class`를 우선 검토한다.
- 트랜잭션 경계는 UseCase 메서드에 명시한다.
- `port.out`을 통해서만 DB/외부 시스템에 접근한다.

### Controller / Infrastructure
- Web Adapter는 DTO 검증/매핑만, 도메인 규칙 구현 금지.
- Persistence Adapter는 엔티티와 도메인 객체 매핑 책임만 가진다.
...
- 기본은 `val`, 필요한 경우에만 `var`
- 실패 모델은 `sealed interface`/`sealed class`를 우선 고려
- 포맷터는 `ktlint`(또는 `ktfmt`) 중 하나로 고정하고 혼용 금지
...
- CI 정적 검증 기본 세트:
  - ktlint (포맷/스타일)
  - detekt (코드 스멜/규칙)
  - SpotBugs (JVM 버그 패턴, 필요 시)

## 10) PR 체크리스트
- [ ] 애그리거트 경계와 트랜잭션 경계가 일치한다.
- [ ] 도메인 계층에 프레임워크 의존이 없다.
- [ ] UseCase는 포트를 통해서만 외부 의존을 사용한다.
- [ ] 아키텍처 테스트(ArchUnit)와 정적 분석(ktlint/detekt)을 통과했다.
- [ ] 코드 포맷이 단일 포맷터(ktlint 또는 ktfmt)로 정렬되었다.

## 11) 도입 순서(권장)
1. 포맷 표준 고정: ktlint(또는 ktfmt 중 하나)
2. 최소 규약 도입: detekt
