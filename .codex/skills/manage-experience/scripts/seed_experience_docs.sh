...
for skill in "${SKILLS[@]}"; do
  ...
  case "${skill}" in
    ...
    plan-sprint)
      title="Plan Sprint"
      e1_situation="계획과 회고를 다른 문맥에서 관리해 학습 루프가 끊긴 상황"
      e1_signal="같은 실패가 다음 스프린트에서 반복됨"
      e1_choice="plan/status/us-retro/sprint-retro를 같은 루프 경로로 관리"
      e1_result="다음 loop-vN+1 진입 판단이 빨라짐"
      e1_condition="스프린트 종료 후 즉시 다음 라운드를 설계할 때"
      e2_situation="US별 Step을 과도하게 세분화해 문서 유지비가 커진 상황"
      e2_signal="진행보다 문서 업데이트 시간이 길어짐"
      e2_choice="실행 필수 Step만 유지하고 나머지는 진행 중 보강"
      e2_result="계획 변경 비용이 감소"
      e2_condition="요구사항 변동성이 큰 스프린트"
      e3_situation="회고에서 원인보다 결과만 기록한 상황"
      e3_signal="Drop 대상 전략이 다음에도 다시 등장"
      e3_choice="False 가정에 원인/증적을 필수 기입"
      e3_result="폐기 전략이 재등장하는 빈도가 줄어듦"
      e3_condition="유사 실패가 2회 이상 반복될 때"
      ;;
    design-implementation)
    ...
    monitor-sprint)
      ...
      e1_situation="진행률 숫자만 보고 일정 리스크를 놓친 상황"
      e1_signal="완료율은 높지만 US 목표일을 넘김"
      e1_choice="진행률과 US 일정 상태(Ahead/On-track/Delayed)를 함께 확인"
      e1_result="지연 신호를 조기 감지"
      e1_condition="US별 완료 목표일이 정의된 스프린트"
      e2_situation="In Progress 표기를 팀마다 다르게 써 파싱이 흔들린 상황"
      e2_signal="동일 태스크가 To Do/진행 중으로 번갈아 집계됨"
      e2_choice="In Progress 표기 규칙을 스프린트 시작 시 합의"
      e2_result="상태 집계 일관성 향상"
