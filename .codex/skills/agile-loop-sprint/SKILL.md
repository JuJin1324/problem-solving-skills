...
description: "Sprint 계획/진행/완료를 한 흐름으로 운영하는 메인 스킬. 실행-검증-학습 루프로 Sprint 결과를 2W/1H Planning 다음 버전에 반영한다."
---

# Agile Loop Sprint

## 목적
Sprint를 계획하고 실행/점검/완료까지 운영한 뒤, 결과를 다음 `2W vN+1 / 1H Planning vN+1`에 반영하는 **학습 루프**를 강제한다.
...
- `references/philosophy.md`
  - 왜 매 스프린트 종료 후 2W/1H Planning 재진입이 필요한지 확인할 때
  - 실행-학습 루프 운영 원칙을 팀에 설명할 때

## 권장 실행 순서
1. Plan: `1h-vN.md` 기반으로 `plan.md` 생성/확정 (US별 Step 작성 포함)
2. Implementation Design: `/implementation-design` 실행
3. Implementation Exec: `/implementation-exec` 실행
4. Test Design: `/test-design` 실행
5. Test Exec: `/test-exec` 실행
6. Status: 진행률과 일정 리스크를 `status.md`에 기록
7. US Retrospective: 각 US 완료 후 `us-N.M-retrospective.md` 작성
8. Sprint Retrospective: `loop-review-vN.md` 작성
9. 다음 라운드 결정
...
- 2W는 유지, 설계(1H Planning)만 수정: `/agile-loop-1h-planning`으로 재진입 (`1h-vN+1.md`)
- 목표 달성: 루프 종료
...
## 다음 단계
- 결정에 따라 `/agile-loop-2w` 또는 `/agile-loop-1h-planning`
