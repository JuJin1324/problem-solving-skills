...
## 목적
대화에서 내려진 기술/설계/범위 결정을 재검토 가능한 문서 증적으로 남겨, 이후 변경 시 판단 근거를 추적할 수 있게 한다.
...
- ADR은 결론만 적지 않고 결정 배경과 트레이드오프를 함께 기록한다.
- 상태 변경(Deprecated/Superseded)은 기존 맥락을 지우지 않고 연결 정보로 남긴다.
...
→ `design-phase.md`, `design-implementation.md` 또는 관련 문서에 참조를 추가하시겠습니까?
```
