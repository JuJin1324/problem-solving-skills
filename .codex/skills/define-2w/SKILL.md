...
- 사용자에게 브리핑 결과 문서 경로를 전달하고, 문서의 `AI 추천`/근거를 리뷰한 뒤 선택하도록 요청한다.
- 사용자가 요청하면 핵심 요약(2~3줄)은 제공할 수 있지만, 선택 기준은 문서 내용에 둔다.
- 브리핑 문서를 생략한 채 선택만 받으면 안 된다.
...
고정 브리핑 스크립트(2줄):
1. `[AI 의견] 유지할 점: {keep_points}. 바꿀 점: {change_points}.`
2. `[입력 요청]`
   - 연속 진행: `위 맥락을 기준으로 지금 바꾸고 싶은 점/유지할 점/추가하고 싶은 점을 자유롭게 말해달라. 추가 입력이 있으면 말해달라. 추가 입력 여부와 상관없이 마지막에는 '다음 진행하자'라고 말해달라.`
   - 새로 시작: `직전 설계는 참고로만 두고, 이번에 새로 정의할 문제를 자유롭게 말해달라. 추가 입력이 있으면 말해달라. 추가 입력 여부와 상관없이 마지막에는 '다음 진행하자'라고 말해달라.`
...
- 브리핑 문서 리뷰 기반 선택 기록(리뷰 요청 메시지 + 사용자 코멘트 + 선택)
- 이전 phase 문서가 있으면 사전 브리핑 기록(AI 의견 + 입력 요청 + 사용자 반응)
- 사용자 자유 입력 원문
...
- 이전 phase 문서 존재 시 브리핑 문서 리뷰 요청 + 선택 완료
