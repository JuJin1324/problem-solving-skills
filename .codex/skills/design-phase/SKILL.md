...
- 설계 완료 후 구현 계열 단계로 넘기기 전 개인 숙고(예: 짧은 산책/메모 정리) 또는 동료 설계 검토 시간을 가진다.
...
### 7단계. 설계 숙성 게이트
- 구현 계열 스킬로 라우팅하기 전에 아래 중 최소 1개를 수행한다:
  - 개인 숙고: 짧은 산책/메모 정리로 설계 리스크와 누락 항목 재점검
  - 동료 검토: 동료와 설계 핵심 가정/경계/리스크를 짧게 점검
- 숙성 게이트 결과(`GO/HOLD`, 핵심 코멘트 1~3개)를 설계 문서 또는 연계 문서에 남긴다.
...
- 설계 직후 숙성 게이트 없이 다음 구현 계열 단계로 즉시 전환함
...
## 다음 단계
- 구현 계열 단계로 진행할 때는 설계 숙성 게이트를 먼저 통과한다.
