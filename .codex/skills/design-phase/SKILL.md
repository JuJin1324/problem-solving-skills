---
name: design-phase
description: "2W vN을 기반으로 1페이즈 분량의 Phase+US/지표/완료조건을 설계하는 메인 스킬. 구현 Step은 design-implementation에서 작성한다."
---
...
- Step 상세는 `/design-implementation`에서 작성하고, Sprint 상태 관리는 `/monitor-sprint`에서 수행한다.
- 1페이즈(스프린트) 범위를 강제한다.
...
- `.agile/loops/loop-vN/01-03-define-2w-case-study.md` (선택)
- `.agile/loops/loop-vN/01-04-define-2w-patterns.md` (선택)
...
- 선행 의존사항
- Sprint 상태 문서 초기화/갱신 책임: `/monitor-sprint` 단계
- 구현 Step 작성 책임: `/design-implementation` 단계
...
## 완료 조건
- `.agile/loops/loop-vN/02-design-phase.md` 작성 완료
...
## 다음 단계
- `/monitor-sprint`로 `02-sprint-status.md` 초기화 후 `/design-implementation` 실행
