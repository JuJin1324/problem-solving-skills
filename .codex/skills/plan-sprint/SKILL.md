...
## 권장 실행 순서
1. Plan: `.agile/loops/loop-vN/04-design-phase.md` 기반으로 `01-sprint-plan.md` 생성/확정 (US별 Step 작성 포함)
...
## 다음 라운드 결정 규칙
- 2W 자체가 바뀜: `/define-2w`로 재진입 (`01-define-2w.md (다음 loop-vN+1 디렉터리)`)
- 2W는 유지, 설계(Phase)만 수정: `/design-phase`로 재진입 (`04-design-phase.md (다음 loop-vN+1 디렉터리)`)
- 목표 달성: 루프 종료
